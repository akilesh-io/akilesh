// Reference only: not built, not routed, not executed.
//
// These helpers backed the custom Notion blog and were moved verbatim out of
// lib/notion.ts when pages/blog was archived (see ../../README.md). They are
// kept so the blog can be restored; blog content now lives on a third-party
// platform.
//
// Note for a future restore: `notion.databases.query` was removed in
// @notionhq/client v5 (the current dependency) in favour of data sources, so
// these queries need porting before they will type check or run again.
